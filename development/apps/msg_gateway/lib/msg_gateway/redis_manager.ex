defmodule MsgGateway.RedisManager do
  @moduledoc false
  ...
  def command(command) when is_list(command) do
    pool_size = String.to_integer(Application.get_env(:msg_gateway, MsgGateway.RedisManager)[:pool_size])
  ...
  def delete_from_redis() do
    {:ok, res} = MsgGateway.RedisManager.keys("*")
    del(res)
  end
