defmodule MsgGateway.Plugs.Headers do
  @moduledoc """
  Plug.Conn helpers
  """

  use MsgGatewayWeb, :plugs
  ...
  defp check_authorization_params("Bearer" <> " " <> <<user::binary-size(16)>> <> ":" <> key_hash, conn) do
    file_name = Application.get_env(:msg_gateway, MsgGatewayWeb.KeysController)[:dets_file_name]
  ...
  defp check_user_status([{_user, {key, true, _, _}}], key_hash, conn) do
  ...
  defp check_authorization_keys(_our_key, _key_hash, conn), do: response_error(conn,  @incorrect_key)
