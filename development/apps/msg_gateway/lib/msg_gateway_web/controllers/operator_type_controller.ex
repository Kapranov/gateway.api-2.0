defmodule MsgGatewayWeb.OperatorTypeController do
  @moduledoc false

  use MsgGatewayWeb, :controller

  alias Core.{
    OperatorTypesRequests,
    OperatorsRequests
  }
  ...
  def update_priority(conn, %{"resource" => operator_info}) do
    with {_n, _new_priority} <- OperatorTypesRequests.update_priority(operator_info)
      do
      RedisManager.set("operators_config",  MsgGatewayInit.create_operators_list_to_redis())
      render(conn, "create.json", %{status: "success"})
    end
  end
