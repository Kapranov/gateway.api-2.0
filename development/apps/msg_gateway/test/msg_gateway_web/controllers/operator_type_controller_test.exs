defmodule MsgGatewayWeb.OperatorTypeControllerTest do
  ...
  defp update_priority_of_operator_type(update_params, conn) do
    ...
    post(conn, "/api/operator_type/update_priority" ,%{
      "resource" =>  [%{
        "active" => true,
        "id" => get_in(update_params, ["id"]),
        "name" => get_in(update_params, ["name"]),
  ...
  defp deactivate_operator_type(operator_type_info, conn) do
    post(conn, "/api/operator_type/deactivate/", %{
      "resource" => %{
