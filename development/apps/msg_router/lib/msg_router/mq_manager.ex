defmodule MsgRouter.MqManager do
  use GenServer
  use AMQP
  ...
  def connect(%{queue_name: queue_name} = state) do
    config = Application.get_env(:msg_router,  MsgRouter.MqManager)
    host = config[:mq_host]
    ...
    case Connection.open([host: host, port: port]) do
      ...
      {:ok, sub} = AMQP.Queue.subscribe(chan, queue_name, fn(payload, _meta) -> MsgRouter.send_message(Jason.decode!(payload, [keys: :atoms])) end)
